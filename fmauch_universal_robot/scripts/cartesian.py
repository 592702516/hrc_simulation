    ...
    if len(contours) > 0:
        ...
        if radius > 10:
            ...
            if real_world_x is not None:
                ...
                target_position = [real_world_x, real_world_y, real_world_z + 0.1]
...
def move_cartesian_path(group, waypoints):
    ...
    (plan, fraction) = group.compute_cartesian_path(
        ...
        eef_step=0.1,        # 末端执行器的步长，单位为米
        avoid_collisions=True # 可选参数：是否避免碰撞
    )
    
    # 检查规划的成功率
    if fraction > 0.9:
        rospy.loginfo(f"笛卡尔路径规划成功，成功率为: {fraction}")

        # 更新每个点的时间戳，确保 time_from_start 严格递增
        time_increment = 0.5  # 每个路径点递增 0.5 秒（可以根据需要调整）
        ...
        rospy.logwarn(f"笛卡尔路径规划失败，成功率为: {fraction}")
...
def main():
    ...
    try:
        ...
        # Close the gripper (you can replace this with your gripper command)
