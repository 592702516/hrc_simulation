...
class IntegratedToolManipulationNode:
    def __init__(self):
        ...
        self.detected_tools = {}  # Dictionary to store initial tool positions
    ...
    def detect_tool_position(self, selected_class_id):
        ...
        for box in results[0].boxes:
            if int(box.cls[0]) == selected_class_id:
                ...
                # Store initial tool position if not already stored
                if selected_class_id not in self.detected_tools:
                    self.detected_tools[selected_class_id] = (tf_x, tf_y)
                return tf_x, tf_y
        return None, None
            ...
            try:
                ...
                while True:
                    ...
                    if gesture == "palm" and self.hand_in_boundary:
                        ...
                        if input_number in self.detected_tools:
                            x, y = self.detected_tools[input_number]
                            self.move_robot([x, y, 0.1335, 2.231, -2.216, 0.0], False, 0.5)
                            self.open_gripper()  # Assuming we're dropping the tool back
                            self.move_robot_home()
                        break
                    rate.sleep()
            except ValueError:
