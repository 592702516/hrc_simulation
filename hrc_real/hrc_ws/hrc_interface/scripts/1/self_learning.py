...
class IntegratedToolManipulationNode:
    def __init__(self):
        ...
        self.tool_positions = {}
        self.rl = ReinforcementLearning([1, 2, 3], [3, 5, 6], epsilon=0.1, learning_rate=0.2)
    ...
    def get_random_class_id(self, input_number):
        return self.rl.choose_action(input_number)
    ...
    def pick_tool(self, input_number):
        ...
        if x is not None and y is not None:
            self.tool_positions[input_number] = (x, y, 0.1335)  # Store the tool position
    ...
    def handle_gesture(self, input_number):
        ...
        if gesture == "palm" and self.hand_in_boundary:
            ...
            self.open_gripper()
            self.move_robot_home()
            self.rl.update_probabilities(input_number, selected_class_id, 1)  # Positive feedback for ε-greedy update
            return True
        elif gesture == "fist":
            if input_number in self.tool_positions:
                x, y, z = self.tool_positions[input_number]
                self.move_robot([x, y, z, 2.231, -2.216, 0.0], False, 0.5)
                self.open_gripper()  # Drop the tool
                self.move_robot_home()  # Return home
                self.rl.update_probabilities(input_number, selected_class_id, 0)  # Negative feedback for ε-greedy update
                return True
        return False
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            try:
                ...
                if not self.pick_tool(input_number):
                    rospy.loginfo("Failed to pick up tool. Retrying...")
                    continue
                
                while True:
                    if self.handle_gesture(input_number):
                        break
                    rate.sleep()
            except ValueError:
...
class ReinforcementLearning:
    def __init__(self, tasks, actions, epsilon=0.1, learning_rate=0.2, save_file="learning_data.json"):
        self.tasks = tasks  # Tasks: [1, 2, 3]
        self.actions = actions  # Actions: [3, 5, 6]
