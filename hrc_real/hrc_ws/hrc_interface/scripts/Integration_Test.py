...
from joint_position_movement import Avoidance
import logging
...
home = [0.297, -0.132, 0.250, 2.231, -2.216, 0.0]

def callback_ag(data):
    global goal_active
    goal_active = data.data

def send_data(new_pos):
    pos = Float32MultiArray()
    pos.data = new_pos
    pub_rp.publish(pos)


def send_pos_and_wait(new_pos):
    send_data(new_pos)
    rospy.sleep(0.8)
...
class YOLOv8ROS:
    def __init__(self):
        ...
        self.image_rate = rospy.Rate(1.5)
...
def pick_up(current_pos):
    ...
    send_pos_and_wait(new_pos)
    rospy.sleep(0.8)
    while goal_active:
        print("waiting")
        rospy.sleep(0.1)
    # avoidance.potential_field(new_pos)
...
def hand_over():
    ...
    if yolov8_ros.rhx:
        ...
        # If outside boundaries, send robot to the edge
        if ((x > 0.24) and (x < 0.80) and (y > -0.48) and (y < 0.43) and (z > 0.25) and (z < 0.6)):   # z > 0.25 with gripper, 0.15 without
            print("Safe to move")

            right_hand = np.array([x, y, z, 2.231, -2.216, 0.0])
            send_pos_and_wait(right_hand)
            # avoidance.go_home()
        ...
        if yolov8_ros.desired_tool:
            ...
            if ((tf_x > 0.24) and (tf_x < 0.88) and (tf_y > -0.48) and (tf_y < 0.6)):
                ...
                if((abs(yolov8_ros.prev_pos[0] - robot_coords[0]) > 0.05) or (abs(yolov8_ros.prev_pos[1] - robot_coords[1]) > 0.05)):
                    send_pos_and_wait(robot_coords)
                    # avoidance.potential_field(robot_coords)
...
if __name__ == '__main__':
    ...
    pub_rp = rospy.Publisher('required_pos', Float32MultiArray, queue_size=10)

    sub_ag = rospy.Subscriber("active_avoidance_goal", Bool, callback_ag)
    # robot = UR5eRobot()
    # avoidance = Avoidance()
    rospy.sleep(1)
